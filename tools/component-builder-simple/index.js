 ...
 * @returns
 */
async function processCSS(inputFiles, output, config = {}) {
...
async function buildCSSThemes(package) {
	...
	return Promise.all(
		fg.sync([
			...
			processCSS(file, `dist/${file}`, { cwd: package, splitinatorConfig: { noSelectors: true } })
		)
	);
}
...
const builder = (package) => Promise.all([
...
module.exports = Promise.all(inputs.map(builder));
